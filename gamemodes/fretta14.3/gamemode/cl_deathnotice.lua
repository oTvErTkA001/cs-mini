...
---------------------------------------------------------*/
function GM:AddDeathNotice(attacker, att_team, inflictor, victim, vic_team)

	if ( !IsValid( g_DeathNotify ) ) then return end

	local pnl = vgui.Create( "GameNotice", g_DeathNotify )
	if attacker then
		pnl:AddText( attacker , team.GetColor(att_team))
		pnl:AddIcon( inflictor )
		pnl:AddText( victim, team.GetColor(vic_team))
	else
		pnl:AddIcon(inflictor)
		pnl:AddText(victim, team.GetColor(vic_team))

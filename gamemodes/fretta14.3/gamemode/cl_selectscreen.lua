...
function PANEL:Init()
	...
	self.ModelShow = vgui.Create("DModelPanel", self)
	self.ModelShow:Dock(RIGHT)
	function self.ModelShow:LayoutEntity(ent)
...
/*---------------------------------------------------------
  AddSelectButton
---------------------------------------------------------*/
function PANEL:AddSelectButton( strName, fnFunction, txt, classes, color )
	...
	if ( txt ) then
		btn.OnCursorEntered = function() self.OldHoverText = self:GetHoverText() self:SetHoverText( txt )
			if !classes then
				self.ModelShow:SetModel("")
			end
			if classes then
				local model = player_class.Get(table.Random(classes)).PlayerModel
				if type(model) == "string" then
					self.ModelShow:SetModel(model)
				end
				if type(model) == "table" then
					self.ModelShow:SetModel(table.Random(model))
				end
				if color then
					function self.ModelShow.Entity:GetPlayerColor() return color:ToVector() end
				end
			end 
		end
...
function PANEL:PerformLayout()
	...
	--self.ModelShow:DockMargin(ScrW()*0.05,ScrW()*0.05,ScrH()*0.2,ScrH()*0.05)
	self.ModelShow:SetSize(ScrW()*0.25,ScrH()*0.25)
	...
	if ( !IsValid( TeamPanel ) ) then 
		...
		for ID, TeamInfo in SortedPairs ( AllTeams ) do
			...
			if ( ID != TEAM_CONNECTING && ID != TEAM_UNASSIGNED && ( ID != TEAM_SPECTATOR || GAMEMODE.AllowSpectating ) && team.Joinable(ID) ) then
				...
				local classes
				if ID < 10 then
					classes = team.GetClass(ID)
				end
				
				local strName = TeamInfo.Name
				local func = function() RunConsoleCommand( "changeteam", ID ) end
			
				local btn = TeamPanel:AddSelectButton( strName, func,"", classes, TeamInfo.Color)
